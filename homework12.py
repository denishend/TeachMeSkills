...
class Product:
    ...
    def __add__(self, other):
        if isinstance(other, Product):
            return sel.__price + other.__price
...
class Warehouse:
    ...
    def sort_by_shop(self):
        ...
        self.__products.sort(key=lambda p: p.shop)

    def sort_by_price(self):
        # TODO: отсортировать по цене
        self.__products.sort(key=lambda p: p.__price)
...
class Bus:
    def __init__(self, max_seats, max_speed, speed=0):
    ...
    def board(self, *passengers):
        ...
        for p in passengers:
            if len(self.passengers) < self.max_seats:
                ...
                self.seats[len(self.passengers)] = p
